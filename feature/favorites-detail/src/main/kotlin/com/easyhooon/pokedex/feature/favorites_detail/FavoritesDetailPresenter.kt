...
class FavoritesDetailPresenter @AssistedInject constructor(
    ...
    override fun present(): FavoritesDetailUiState {
        ...
        return FavoritesDetailUiState(
            isLoading = isLoading,
            pokemon = screen.pokemon,
        ) { event ->
            when (event) {
                is FavoritesDetailUiEvent.OnBackClick -> navigator.pop()
                is FavoritesDetailUiEvent.OnFavoritesButtonClick -> removeFavoritePokemon()
            }
        }
    }
